...
do
    ...
    function Funcs:AddDropdown(Idx, Info)
        ...
        function Dropdown:BuildDropdownList()
            ...
            -- Sort values into regular and disabled
            for _, Value in pairs(Values) do
                ...
                if table.find(DisabledValues, Value) then
                    table.insert(DisabledItems, Value)
                else
                    table.insert(RegularValues, Value)
                end
            end

            -- First add regular values
            for _, Value in pairs(RegularValues) do
                Count += 1
                local Table = {}
            ...
            -- Then add disabled values
            for _, Value in pairs(DisabledItems) do
                Count += 1
                local Table = {}
                ...
                function Table:UpdateButton() end
                Buttons[Button] = Table
            end
...
function Library:CreateWindow(Info)
    ...
    -- Set fixed size - this will be the smallest usable size
    local FixedSize = UDim2.fromOffset(480, 360)
    
    local Window = {
        NameIndicator = ScreenGui.Name .. WindowInfo.Title,
    }
    Library:UpdateDPI(ScreenGui, {
        Position = false,
        Size = false,
    })
    ...
    MainFrame = New("Frame", {
        ...
        Name = "Main",
        Position = UDim2.new(0.5, -FixedSize.X.Offset / 2, 0.5, -FixedSize.Y.Offset / 2),
        Size = FixedSize,
        Visible = false,
        Parent = ScreenGui,
    ...
    --// Resize Button
    -- Removed resize button and functionality to enforce fixed size
    ...
    -- Replace resize icon with a footer note
    New("TextLabel", {
        AnchorPoint = Vector2.new(1, 0.5),
        BackgroundTransparency = 1,
        Position = UDim2.new(1, -10, 0.5, 0),
    ...
    Library:GiveSignal(SearchBox:GetPropertyChangedSignal("Text"):Connect(function()
        ...
        -- The previous search code used to be here but has been removed
    end))
